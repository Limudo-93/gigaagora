...
async function processQueue() {
  ...
  for (const item of queue || []) {
    processed += 1;
    console.log(
      `[Notifications Process] Processing notification ${item.id} for user ${item.user_id}, type: ${item.notification_type}`,
    );
    ...
    if (subError) {
      console.error(
        `[Notifications Process] Error fetching subscriptions for user ${item.user_id}:`,
        subError,
      );
    }
    ...
    if (subError || !subscriptions || subscriptions.length === 0) {
      console.log(
        `[Notifications Process] No subscriptions found for user ${item.user_id}, skipping (user hasn't enabled push notifications)`,
      );
      await supabase
        .from("push_notification_queue")
        .update({
          ...
          last_error:
            subError?.message ||
            "Skipped: User has no active push subscriptions",
        })
        .eq("id", item.id);
      skipped += 1;
      continue;
    ...
    console.log(
      `[Notifications Process] Found ${subscriptions.length} subscription(s) for user ${item.user_id}`,
    );
    ...
    for (const sub of subscriptions) {
      ...
      try {
        console.log(
          `[Notifications Process] Sending notification to subscription ${sub.endpoint.substring(0, 50)}...`,
        );
        ...
        if (sendError) {
          console.error(
            `[Notifications Process] Error sending to subscription:`,
            sendError,
          );
          console.error(
            `[Notifications Process] Error type:`,
            typeof sendError,
          );
          console.error(
            `[Notifications Process] Error keys:`,
            Object.keys(sendError || {}),
          );
          ...
          if (errorContext) {
            console.error(
              `[Notifications Process] Error context:`,
          ...
          const parsedStatus = Number(
            parsedBody?.statusCode ?? errorDetails.statusCode,
          );
          if (
        ...
        if (data && typeof data === "object" && "error" in data) {
          ...
          const errorDetails =
            (data as any).details || (data as any).body || "";
          console.error(
            `[Notifications Process] Edge function returned error:`,
            errorMsg,
          );
        ...
        console.log(
          `[Notifications Process] Successfully sent notification to subscription`,
        );
        successCount += 1;
      } catch (err: any) {
        ...
        console.error(
          `[Notifications Process] Catch block - Error type:`,
          typeof err,
        );
        console.error(
          `[Notifications Process] Catch block - Error keys:`,
          Object.keys(err || {}),
        );
        ...
        try {
          ...
          console.error(
            `[Notifications Process] Catch block - Full error:`,
            errorString,
          );
        } catch (e) {
          console.error(
            `[Notifications Process] Catch block - Error (string):`,
            String(err),
          );
        }
        ...
        if (err?.response) {
          try {
            ...
            if (responseText) {
              console.error(
                `[Notifications Process] Response text:`,
                responseText,
              );
              try {
                ...
                console.error(
                  `[Notifications Process] Parsed error JSON:`,
      ...
      await supabase
        ...
        .update({
          ...
          next_attempt_at: new Date(
            Date.now() + RETRY_MINUTES * 60 * 1000,
          ).toISOString(),
...
async function handler(request: Request) {
  ...
  const isAuthorized =
    isCron || (secret && auth === `Bearer ${secret}`) || !secret;
