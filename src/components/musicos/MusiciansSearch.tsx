    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        const response = await fetch(
          `/api/musicos/nearby?${params.toString()}`,
        );
        if (response.ok) {
          ...
          <Card className="card-glass md:col-span-2 xl:col-span-3">
            <CardContent className="p-8 text-center">
              <p className="text-sm text-foreground/70">
                Nenhum músico encontrado com esses filtros. Tente ajustar a
                busca ou explore outros instrumentos.
              </p>
            </CardContent>
          </Card>
        ) : (
          results.map((musician) => {
            ...
            return (
              <Card
                ...
                className="card-glass hover:shadow-xl transition-shadow"
              >
                <CardContent className="p-5 space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Button asChild className="btn-gradient flex-1">
                      <Link href={profileHref as any}>Ver perfil</Link>
                    </Button>
                    <Button
                      variant="outline"
                      asChild
