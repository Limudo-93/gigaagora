...
export default function PendingInvites({ userId }: { userId: string }) {
  ...
  const [acceptedInviteTitle, setAcceptedInviteTitle] = useState<string | null>(
    null,
  );
  ...
  const fetchPending = useCallback(async () => {
    ...
    try {
      ...
      const musicianLat = musicianProfile?.latitude as
        | number
        | null
        | undefined;
      const musicianLng = musicianProfile?.longitude as
        | number
        | null
        | undefined;
      ...
      if (musicianLat != null && musicianLng != null && radius != null) {
        ...
        console.log(
          `[PendingInvites] Filtrando convites por raio: ${radiusValue} km (tipo: ${typeof radiusValue}). Total antes: ${transformed.length}`,
        );
        ...
        transformed = transformed.filter((item) => {
          ...
          if (item.distance_km == null) {
            console.log(
              `[PendingInvites] Excluindo convite ${item.invite_id} - sem distância calculada`,
            );
            return false;
          }
          ...
          if (!isWithinRadius) {
            console.log(
              `[PendingInvites] Excluindo convite ${item.invite_id} - distância ${distance.toFixed(1)} km > raio ${radiusValue} km`,
            );
          } else {
            console.log(
              `[PendingInvites] Mantendo convite ${item.invite_id} - distância ${distance.toFixed(1)} km <= raio ${radiusValue} km`,
            );
          }
          return isWithinRadius;
        });
        console.log(
          `[PendingInvites] Total após filtro: ${transformed.length} (removidos: ${beforeFilter - transformed.length})`,
        );
      } else {
        console.log(
          `[PendingInvites] Não filtrando por raio - musicianLat: ${musicianLat}, musicianLng: ${musicianLng}, radius: ${radius} (tipo: ${typeof radius})`,
        );
      }
    ...
    async (inviteId: string) => {
      ...
      try {
        ...
        if (rpcData && typeof rpcData === "object" && "ok" in rpcData) {
          if (!rpcData.ok) {
            const message =
              (rpcData as any)?.message || "Erro ao aceitar convite";
  ...
  const declineInvite = useCallback(
    async (inviteId: string, reason: DeclineReason) => {
      setBusyId(inviteId);
      setErrorMsg(null);
      ...
      try {
        ...
        if (
          ...
          const message =
            (rpcData as any)?.message || "Erro ao recusar convite";
          console.error("RPC returned error:", message);
          setErrorMsg(message);
          setBusyId(null);
          return;
    ...
    <section className="mt-4 md:mt-6">
      <div className="flex items-center justify-between mb-3 md:mb-4">
        <h2 className="text-lg md:text-xl font-semibold text-foreground">
          Convites Pendentes{" "}
          {count > 0 && (
            <Badge className="ml-2 text-xs md:text-sm">{count}</Badge>
          )}
        </h2>
      </div>
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              let distanceKm: number | null = null;
                              let estimatedTravelTimeMinutes: number | null =
                                null;
                              ...
                              setErrorMsg(
                                `Erro ao carregar detalhes do convite: ${err.message}`,
                              );
                            }
                          }}
                        >
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <div className="flex items-center gap-3 mb-2">
              ...
              <DialogTitle>Compromisso Confirmado</DialogTitle>
            </div>
            <DialogDescription>
              Você aceitou o convite para{" "}
              <strong>{acceptedInviteTitle || "esta gig"}</strong>
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 mt-4">
            <div className="p-4 bg-primary/10 border-l-4 border-primary rounded-r-lg">
              <p className="text-sm text-foreground leading-relaxed">
                ...
                mas é fundamental que você honre este compromisso. A confiança
                entre músicos e contratantes é essencial para o funcionamento da
                plataforma.
              </p>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 p-3 bg-yellow-500/10 rounded-lg border border-yellow-500/20">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    A frequência de cancelamentos, mesmo com antecedência, pode
                    levar ao bloqueio temporário na plataforma.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-3 bg-destructive/10 rounded-lg border border-destructive/20">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    <strong>
                      Cancelamentos com menos de 24 horas até o início do evento
                    </strong>{" "}
                    resultam em bloqueio de <strong>7 dias</strong> para receber
                    convites de novas gigs.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="pt-2">
              <p className="text-xs text-muted-foreground text-center">
                Ao continuar, você confirma que entendeu e concorda com estas
                políticas.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center gap-3 mt-6">
            {countdown > 0 && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                <span>
                  Por favor, leia as informações acima antes de continuar
                </span>
              </div>
            )}
            <Button
      ...
      {pendingDeclineInviteId && (
        <DeclineReasonDialog
          ...
          gigTitle={
            items.find((x) => x.invite_id === pendingDeclineInviteId)
              ?.gig_title ?? null
          }
